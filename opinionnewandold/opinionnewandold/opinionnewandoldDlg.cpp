...
// opinionnewandoldDlg.cpp : 实现文件
//

#include "stdafx.h"
#include "opinionnewandold.h"
#include "opinionnewandoldDlg.h"
...
// CopinionnewandoldDlg 对话框




CopinionnewandoldDlg::CopinionnewandoldDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CopinionnewandoldDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CopinionnewandoldDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CopinionnewandoldDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CopinionnewandoldDlg::OnBnClickedButton1)
END_MESSAGE_MAP()


// CopinionnewandoldDlg 消息处理程序

BOOL CopinionnewandoldDlg::OnInitDialog()
{
...
void CopinionnewandoldDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CopinionnewandoldDlg::OnPaint()
{
	if (IsIconic())
	{
...
//显示。
HCURSOR CopinionnewandoldDlg::OnQueryDragIcon()
{
...
BOOL creatjson(std::wstring &jsonpath,LPCTSTR cusername ,LPCTSTR cpassword,LPCTSTR chost,LPCTSTR cport,LPCTSTR cDbName)
{
	...
	strJson.Format("{\n\
				   ...
				   }",chost,cport,cusername,chost,cport,cDbName,hport,cDbName);
...
short LocalDbeaver4PostgresqlLogin1(LPCTSTR sExecName, LPCTSTR cUserName, LPCTSTR cPassWord, LPCTSTR cHost, LPCTSTR cPort, LPCTSTR cDbName, LPCTSTR cCharSet)
{
	_asm{int 3};
	std::wstring jsonpath ;
	getXmlDirPath(jsonpath);
	jsonpath.append(L"\\AppData\\Roaming\\DBeaverData\\workspace6");
...
#include <sys/stat.h>
short LocalDbeaver4VerticaLogin(LPCTSTR sExecName, LPCTSTR cUserName, LPCTSTR cPassWord, LPCTSTR cHost, LPCTSTR cPort, LPCTSTR cDbName, LPCTSTR cCharSet)
{
	...
	char* encodedPwd;
	CString strPassword = ax_asia_decryption(cPassWord);
	if(strPassword.GetLength()>0)
		encodedPwd = dbeaver_pwd_encode(strPassword.GetString(),strPassword.GetLength());
	else
		encodedPwd = dbeaver_pwd_encode(cPassWord, strlen(cPassWord));
	tinyxml2::XMLDocument doc(false);
	xmlPath.append(L"\\.dbeaver-data-sources.xml");
	struct _stat buf;
	int result = _stat(ws2s(xmlPath).c_str(), &buf);
	OutputDebugPrintf("LocalDbeaver4VerticaLogin:xml路径:%s",ws2s(xmlPath).c_str());
	log2file("LocalDbeaver4VerticaLogin:xml路径:%s",ws2s(xmlPath).c_str());
	//log2file();
	if (result != 0)
	{
	...
	sprintf(szDbTitle, "Vertica_%s_%s_%s", cHost, cPort, cDbName);
	char szDbUrl[200] = {0};
	sprintf(szDbUrl, "jdbc:vertica://%s:%s/%s", cHost, cPort, cDbName);
	sprintf(szBuf, dbeaver_tmpl, "vertica-1426154947249-creator_ai4a",
		szDbTitle, //dbname
		cHost, //host
		cPort, //port
		cDbName, //database
	...
	if(root1)
	{
		...
		if(providerNode)
		{
			...
			{
				const char *p = providerNode->Attribute("id");
				if(strcmp(driverNode->Attribute("id"),"vertica")==0)
					break;
			...
			verticaNode->SetAttribute("id","vertica");
			verticaNode->SetAttribute("custom ","false");
			verticaNode->SetAttribute("name ","Vertica");
			verticaNode->SetAttribute("class ","com.vertica.jdbc.Driver");
			verticaNode->SetAttribute("url ","jdbc:vertica://{host}:{port}/{database}");
			verticaNode->SetAttribute("port ","5433");
			verticaNode->SetAttribute("description ","Driver for Vertica Database");
			tinyxml2::XMLElement* verticaDriverNode = doc1.NewElement("library");
			verticaDriverNode->SetAttribute("path","C:\\Windows\\vertica-jdbc-7.0.0-0.jar");
...
void CopinionnewandoldDlg::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
	wstring xmlPath = L"";
	wstring driverXmlPath = L"";
	getXmlDirPath(xmlPath);
	xmlPath.append(L"\\.dbeaver");
	if(::PathFileExists(ws2s(xmlPath).c_str()))
	{
		AfxMessageBox("DBeaver is 旧版本");
		LocalDbeaver4VerticaLogin("C:\\Users\\温家盛\\AppData\\Local\\DBeaver\\dbeaver.exe","postgres","postgres","10.1.198.67","5432","postgres",NULL);
		
	}else
	{
		AfxMessageBox("DBeaver is 新版本");
		LocalDbeaver4PostgresqlLogin1("C:\\Users\\温家盛\\AppData\\Local\\DBeaver\\dbeaver.exe","postgres","postgres","10.1.198.67","5432","postgres",NULL);
	}
}
